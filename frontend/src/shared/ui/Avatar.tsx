  ...
  return (
    ...
    >
      ...
      {src && !imageError ? (
        ...
        <div
          className={`rounded bg-brand flex items-center justify-center text-on-brand font-normal shrink-0 ${sizeClasses[size]}`}
        >
      ...
      {hasText && (
        <div className="flex flex-col min-w-0 flex-1 overflow-hidden">
          <div
            className={`font-normal text-text-primary truncate ${textSizeClasses[size].primary} ${primaryTextClassName}`}
          >
            {primaryText}
          </div>
          {secondaryText && (
            <div
              className={`${secondaryTextClassName ? '' : 'text-text-tertiary'} truncate ${textSizeClasses[size].secondary} ${secondaryTextClassName}`}
            >
              {secondaryText}
            </div>
          )}
