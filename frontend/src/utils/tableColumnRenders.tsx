...
import { formatDate, formatPhoneNumber, calculateAge } from '@/utils';
...
export function renderPatientNameBlock(
  fullName: string,
  secondary?: ReactNode
): ReactNode {
  return (
...
export function renderContactBlock(phone: string, email?: string): ReactNode {
  return (
    <div className="text-xs min-w-0 font-normal">
      <div className="text-xs text-text-primary truncate font-normal">{formatPhoneNumber(phone)}</div>
      {email ? (
...
export function renderDateCell(date: string | Date | null | undefined, emptyLabel = '—'): ReactNode {
  if (!date) {
