...
function buildSimpleViewConfig(): TableViewConfig<Order> {
  const columns = [
    ...
    },
    {
      ...
      width: 'md' as const,
      render: (order: Order) => (
        <span className="text-xs text-fg-subtle">{formatDetailDate(order.orderDate, 'short')}</span>
      ),
    },
    {
      key: 'overallStatus',
  ...
  const CardComponent: React.FC<CardComponentProps<Order>> = ({ item, onClick }) => (
    ...
    >
      ...
      <div className="text-xs text-fg-subtle mt-1">{formatDetailDate(item.orderDate, 'short')}</div>
    </div>
  );
  return {
    fullColumns: columns,
...
function buildDetailedViewConfig(): TableViewConfig<Order> {
  const fullColumns = [
    ...
    },
    {
      ...
      width: 'md' as const,
      render: (order: Order) => (
        <span className="block truncate text-xs text-fg-subtle">{formatDetailDate(order.orderDate, 'short')}</span>
      ),
    },
    {
      key: 'tests',
