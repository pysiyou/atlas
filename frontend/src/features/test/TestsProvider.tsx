...
export const TestsProvider: React.FC<TestsProviderProps> = ({ children }) => {
  ...
  const updateTest = useCallback(
    async (code: string, updates: Partial<Test>) => {
      try {
   ...
   */
  const getTestsByCategory = useCallback(
   ...
   */
  const searchBySynonym = useCallback(
    (synonym: string): Test[] => {
      ...
      return tests.filter(
        test =>
          test.isActive && test.synonyms?.some(syn => syn.toLowerCase().includes(lowerSynonym))
      );
    },
    [tests]
  );
   ...
   */
  const getTestsByPanel = useCallback(
    (panelName: string): Test[] => {
      if (!panelName.trim()) return [];

      const lowerPanel = panelName.toLowerCase();
      return tests.filter(
        test =>
          test.isActive && test.panels?.some(panel => panel.toLowerCase().includes(lowerPanel))
      );
    },
    [tests]
  );
   ...
   */
  const getTestByLoincCode = useCallback(
    (loincCode: string): Test | undefined => {
      if (!loincCode.trim()) return undefined;

      const lowerLoinc = loincCode.toLowerCase();
      return tests.find(
        test => test.isActive && test.loincCodes?.some(loinc => loinc.toLowerCase() === lowerLoinc)
      );
    },
    [tests]
  );
   ...
   */
  const getTestsBySampleRequirements = useCallback(
    (sampleType: string): Test[] => {
      if (!sampleType.trim()) return [];

      const lowerSampleType = sampleType.toLowerCase();
      return tests.filter(
        test => test.isActive && test.sampleType.toLowerCase() === lowerSampleType
      );
    },
    [tests]
  );
   ...
   */
  const updateTestPrice = useCallback(
    async (code: string, price: number) => {
      if (price < 0) {
   ...
   */
  const toggleTestActive = useCallback(
    async (code: string) => {
