...
export const ProductivityTable: React.FC<ProductivityTableProps> = ({ data, compact = false }) => {
  if (data.byTechnician.length === 0) {
    if (compact)
      return <div className="flex items-center justify-center min-h-[120px]">{emptyContent}</div>;
    return (
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {data.byTechnician.slice(0, 10).map(tech => (
            <tr
              key={tech.userId}
              className="border-b border-border-default/50 hover:bg-surface-hover"
            >
              <td className="py-2 px-3 text-text-primary">{tech.userName}</td>
              <td className="py-2 px-3 text-right text-text-secondary">{tech.resultsEntered}</td>
              <td className="py-2 px-3 text-right text-text-secondary">{tech.validations}</td>
