...
export const CreateOrder: React.FC = () => {
  ...
  const numericPatientId = selectedPatientId ? (typeof selectedPatientId === 'string' ? parseInt(selectedPatientId, 10) : selectedPatientId) : null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newOrder: Omit<Order, 'orderId'> & { orderId?: number } = {
        ...
        patientId: typeof selectedPatient.id === 'string' ? parseInt(selectedPatient.id, 10) : selectedPatient.id,
      ...
      // Create order first - backend will assign orderId
      await addOrder(newOrder as Order);
      
      // Refresh orders to get the newly created order with its assigned ID
      await ordersContext.refreshOrders();
      
      // Find the newly created order (by patient and recent timestamp)
      const createdOrder = ordersContext.orders
        .filter(o => o.patientId === newOrder.patientId)
        .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())[0];
      
      const actualOrderId = createdOrder?.orderId || newOrder.orderId;
