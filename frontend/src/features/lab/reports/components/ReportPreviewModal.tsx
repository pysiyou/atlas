        ...
        <div className="flex-1 overflow-y-auto ">
          ...
          <div className="flex border-b border-border">
            ...
            <div className="bg-surface-report p-6 shrink-0 space-y-2" style={{ width: '40%' }}>
              {/* Logo */}
              {/* <div className="mb-4">
                <div className="w-16 h-16 bg-surface rounded-full border border-border flex items-center justify-center">
                  <Icon 
                    name={companyConfig.getBranding().logoPath.replace('/icons/', '').replace('.svg', '') as IconName} 
                    className="w-8 h-8 text-text-2" 
                  />
                </div>
              </div> */}
              ...
              {companyConfig.getConfig().company.subtitle && (
                <p className="text-xs text-text font-semibold">{companyConfig.getConfig().company.subtitle}</p>
              )}
            ...
            <div className="bg-surface p-4 flex-1">
              {/* Report Title */}
              <h1 className="text-base font-bold text-text mb-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className="space-y-0.5">
                  <p className="text-base font-bold text-text">{reportData.patientName}</p>
                  {reportData.patientAge && (
                    <div className="flex gap-2">
                      <span className="text-xs text-text-2 min-w-[50px] truncate">Age:</span>
                      <span className="text-xs font-bold text-text">{reportData.patientAge}</span>
                    </div>
                  )}
                  {reportData.patientGender && (
                    <div className="flex gap-2">
                      <span className="text-xs text-text-2 min-w-[50px] truncate">Gender:</span>
                      <span className="text-xs font-bold text-text">{reportData.patientGender.toUpperCase()}</span>
                    </div>
                  )}
                  ...
                  {(() => {
                    ...
                    if (orderExtended.patientPhone) {
                      return (
                        <div className="flex gap-2">
                          <span className="text-xs text-text-2 min-w-[50px] truncate">Phone:</span>
                          <span className="text-xs font-bold text-text">{orderExtended.patientPhone}</span>
                        </div>
                      );
                    }
                    if (orderExtended.patientEmail) {
                      return (
                        <div className="flex gap-2">
                          <span className="text-xs text-text-2 min-w-[50px] truncate">Email:</span>
                          <span className="text-xs font-bold text-text">{orderExtended.patientEmail}</span>
                        </div>
                      );
                    }
                    return null;
                ...
                <div className="space-y-0.5">
                  <p className="text-base font-bold text-text">Processing Details</p>
                  {reportData.timestamps?.collectedAt || reportData.sampleCollection?.collectedAt ? (
                    <div className="flex gap-2">
                      <span className="text-xs text-text-2 min-w-[60px] truncate">Sample:</span>
                  ...
                  {reportData.timestamps?.reportedAt ? (
                    <div className="flex gap-2">
                      <span className="text-xs text-text-2 min-w-[60px] truncate">Results:</span>
                      <span className="text-xs font-bold text-text">
                  ...
                  {reportData.testResults[0] && (
                    <div className="flex gap-2">
                      <span className="text-xs text-text-2 min-w-[60px] truncate">Verified by:</span>
                      <span className="text-xs font-bold text-text">
                        {(() => {
            ...
            <div key={index} className="space-y-4">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-canvas border-b border-border-strong">
                      <th className="px-6 py-3 text-left font-medium text-text-3 uppercase tracking-wider">Investigation</th>
                      <th className="px-6 py-3 text-left font-medium text-text-3 uppercase tracking-wider">Result</th>
                      <th className="px-6 py-3 text-left font-medium text-text-3 uppercase tracking-wider">Reference Value</th>
                      <th className="px-6 py-3 text-right font-medium text-text-3 uppercase tracking-wider">Unit</th>
                    </tr>
                  </thead>
                  <tbody>
                    {/* Primary Sample Type Row */}
                    <tr className="border-b border-border-subtle hover:bg-canvas transition-colors">
                      <td className="px-6 py-3 text-text">Primary Sample Type :</td>
                      <td className="px-6 py-3 text-left text-text">
                        {reportData.order.tests[0]?.sampleType?.toUpperCase() || 'N/A'}
                      </td>
                      <td className="px-6 py-3 text-left text-text-2"></td>
                      <td className="px-6 py-3 text-right text-text-2"></td>
                    </tr>
                    ...
                    {test.parameters.map((param, paramIndex) => {
                      ...
                      return (
                        <tr
                          key={paramIndex}
                          className={cn(
                            'border-b border-border-subtle last:border-0 hover:bg-canvas transition-colors',
                        ...
                        >
                          <td className={cn(
                            ...
                            {isSectionHeader ? (
                              ...
                              <div className="min-w-0">
                                <div className="font-bold text-text truncate">{param.name}</div>
                                {param.code && (
                          ...
                          <td className={cn(
                            'px-6 py-3 text-left',
                            isAbnormal ? 'text-danger-text font-bold' : 'text-text'
                          )}>
                            {param.value}
                          </td>
                          <td className="px-6 py-3 text-left text-text-2">
                            {param.referenceRange || ''}
                          </td>
                          <td className="px-6 py-3 text-right text-text-2">
                            {param.unit || '-'}
                          </td>
                        </tr>
                      );
