            ...
            <button
              ...
              className={`px-2 py-1 text-xs rounded transition-colors font-mono ${
                activeTestCode === test.code
                  ? 'bg-action-primary-muted-bg text-action-primary font-medium'
                  : 'bg-neutral-100 text-action-primary hover:bg-neutral-200'
              }`}
            >
              {test.code}
            </button>
    ...
    >
      <div className="space-y-3 pt-2 animate-in fade-in duration-200">
        {activeTest.fastingRequired && (
          <div className="flex items-start gap-2 p-2 bg-feedback-warning-bg border border-feedback-warning-border rounded">
            <span className="w-1.5 h-1.5 rounded-full bg-feedback-warning-bg0 mt-1.5 shrink-0" />
            <div className="flex-1">
              <div className="text-xs font-medium text-feedback-warning-text">Fasting Required</div>
              <div className="text-xs text-feedback-warning-text mt-0.5">
        ...
        {activeTest.rejectionCriteria && activeTest.rejectionCriteria.length > 0 && (
          <div className="flex items-start gap-2 p-2 bg-feedback-danger-bg border border-feedback-danger-border rounded">
            <Icon name={ICONS.actions.alertCircle} className="w-4 h-4 text-feedback-danger-text mt-0.5 shrink-0" />
            <div className="flex-1">
              <div className="text-xs font-medium text-feedback-danger-text-strong mb-1">Rejection Criteria</div>
              <ul className="list-disc list-inside space-y-0.5">
                {activeTest.rejectionCriteria.map((criteria, idx) => (
                  <li key={idx} className="text-xs text-feedback-danger-text-strong">
                    {criteria}
                  </li>
                ))}
              </ul>
