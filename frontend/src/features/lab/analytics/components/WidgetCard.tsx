...
export const WidgetCard: React.FC<WidgetCardProps> = ({
  ...
  <Card variant="default" padding="sm" className={cn('rounded-lg shadow-sm flex flex-col', className)}>
    ...
    <div className="flex items-baseline gap-2 flex-wrap">
      <span className="text-2xl font-normal text-text-primary">{value}</span>
      {change !== undefined && (
        <span
          className={cn(
            'text-sm font-normal',
    ...
    {chartTitle && (
      <p className="text-xs font-normal text-text-secondary mt-3 pb-1 border-b border-dotted border-border-default">
        {chartTitle}
      </p>
    )}
