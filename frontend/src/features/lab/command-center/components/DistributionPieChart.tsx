...
export const DistributionPieChart: React.FC<DistributionPieChartProps> = ({
  ...
  return (
    ...
    >
      ...
      {showHeader && <div className="flex items-center justify-between px-4 pt-4 pb-2 shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          <h3 className="text-lg font-bold truncate text-text-primary">
            {title}
          </h3>
          <button
            type="button"
      ...
      <div className="flex-1 min-h-[200px] min-w-0 flex flex-row overflow-hidden">
        {/* Left: donut + center + segment labels */}
        <div className="shrink-0 flex flex-col min-w-0" style={{ width: '52%' }}>
          <div className="flex-1 min-h-0 relative flex items-center">
            ...
            {/* Center text (donut center is at cx 45%) */}
          ...
          <div className="shrink-0 flex flex-col py-2 pr-3 pl-3 overflow-y-auto">
            {listItems.length === 0 ? (
              ...
              listItems.map((item, index) => (
                ...
                >
                  {/* Icon in full color on attenuated same-color background */}
                  <div
                    className="shrink-0 w-9 h-9 rounded flex items-center justify-center"
                    style={{
                      backgroundColor: `color-mix(in srgb, ${item.color ?? COLORS[index % COLORS.length]} 24%, transparent)`,
                      color: item.color ?? COLORS[index % COLORS.length],
                    }}
                  >
                    <Icon
                      name={getStageIcon(item.name)}
                      className="w-5 h-5 [&>svg]:shrink-0"
                    />
                  </div>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {item.change != null ? (
                        <span
                          ...
                          style={{
                            color: item.change >= 0 ? CHART_SUCCESS : CHART_DANGER,
                          }}
                        >
                          {item.change >= 0 ? '+' : ''}
                          ...
                          <span style={{ color: item.change >= 0 ? CHART_SUCCESS : CHART_DANGER }}>
                            <Icon
                              name={item.change >= 0 ? 'up-trend' : 'down-trend'}
